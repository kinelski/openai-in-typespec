...
### "missing file" error in the `.dotnet` or `.plugin` directories

If you get an error about a missing dependant file when building one of the projects under `\dotnet` path:

```bash

error MSB3030: Could not copy the file "..\..\node_modules\@typespec\http-client-csharp\dist\generator\Microsoft.TypeSpec.Generator.runtimeconfig.json" because it was not found.

```

Try first running the script `Invoke-CodeGen.ps1` under `\dotnet\.scripts\` or `\dotnet\.scripts.azure\` to emplace the needed files.
